    ...
    switch (iMsg){
        ...
        case WM_COMMAND:
            ...
            switch (LOWORD (wParam)) {
				...
				case IDM_BLACKWHITE:
                    if (glCurrentBitmap){
                        newBitmap = GetBWBitmap (glCurrentBitmap);
                        UpdateCurrentBitmap (newBitmap, hwnd);
                        }
                    break;
				case IDM_BRIGHT:
                    if (glCurrentBitmap){
                        newBitmap = GetBrightBitmap (glCurrentBitmap);
                        UpdateCurrentBitmap (newBitmap, hwnd);
                        }
                    break;
				case IDM_DARK:
                    if (glCurrentBitmap){
                        newBitmap = GetDarkBitmap (glCurrentBitmap);
                        UpdateCurrentBitmap (newBitmap, hwnd);
                        }
                    break;
                case IDM_MIRROR:
                    if (glCurrentBitmap) {
                        newBitmap = GetmMirorredBitmap(glCurrentBitmap);
                        UpdateCurrentBitmap(newBitmap, hwnd);
                    }
                    break;
                case IDM_BIG9:
                    if (glCurrentBitmap) {
                        newBitmap = GetSizedUp9BITMAP(glCurrentBitmap);
                        UpdateCurrentBitmap(newBitmap, hwnd);
                    }
                    break;
                default:
